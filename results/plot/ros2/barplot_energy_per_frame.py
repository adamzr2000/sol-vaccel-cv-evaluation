...
energy_per_frame_J = (total_energy_J - idle_energy_J) / n_processed_frames
  total_energy_J: cpu_energy_j_total / gpu_energy_j_total (exact hardware-
    counter totals, RAPL/NVML) from system-stats _summary JSONs, not a
    mean(power) x duration approximation.
  idle_energy_J:  idle_power_w * duration_sec, where idle_power_w is the
    exact idle baseline from ../../experiments/system-stats/idle/summary.csv
    (see plot_config.py), so bars show energy attributable to the workload
    rather than the host's always-on draw.
...
def load_frame_counts() -> dict[tuple, int]:
    """
    Returns {(model, backend, run_tag): n_frames_monitor} from model-stats summaries.
    Uses n_frames_monitor — frames processed as counted on the robot (monitor_host),
    matching the host the energy itself (Robot CPU) is measured on. n_frames_pipeline
    counts at the pipeline/inference-node side instead, which for remote-* runs is a
    different host (edge-asus) and can drift slightly from the robot's own count.
    """
    ...
    for tag in ("local-cpu", "remote-cpu", "remote-gpu"):
        ...
        for run in json.loads(path.read_text())["runs"]:
            ...
            # Segmentation models read their Remote CPU/GPU frame count from
            # the override file below instead, when one is configured.
            if tag in seg_overrides and model in SEG_MODELS and seg_overrides[tag]:
                continue
            n = run.get("n_frames_monitor")
            if n:
                counts[(model, backend, rt)] = int(n)

    # Segmentation Remote CPU/GPU override: pull every segmentation model's
    # frame count from the configured summary file, stored under the
    # canonical "remote-cpu"/"remote-gpu" tag (not that file's own run_tag)
    # so load_epf_map()'s lookup -- which also forces that tag for these
    # models -- finds it.
    for tag, override_tag in seg_overrides.items():
        if not override_tag:
            continue
        ov_path = MODEL_DIR / f"{override_tag}_model_stats.json"
        if not ov_path.exists():
            print(f"[WARN] Missing override model-stats: {ov_path}")
            continue
        for run in json.loads(ov_path.read_text())["runs"]:
            model = run.get("model", "")
            if model not in SEG_MODELS:
                continue
            backend = run.get("backend", "")
            n = run.get("n_frames_monitor")
            if n:
                counts[(model, backend, tag)] = int(n)
...
def load_epf_map(frame_counts: dict[tuple, int]) -> tuple[dict[tuple, float], dict[tuple, tuple[float, int, float]]]:
    ...
    own duration - see plot_config.py's idle_host_for()/subtract_idle().
    ...
    def _record(model, label, host, host_data, n):
    ...
    for tag in ("local-cpu", "remote-cpu", "remote-gpu"):
        ...
        for run in json.loads(path.read_text())["runs"]:
            ...
            for host, host_data in run["hosts"].items():
                _record(model, label, host, host_data, n)
    ...
    # run_tag, matched against the override frame count stored under that
    # same tag above.
    for tag, override_tag in seg_overrides.items():
        if not override_tag:
            continue
        ...
        for run in json.loads(ov_path.read_text())["runs"]:
            ...
            if model not in SEG_MODELS:
                continue
            backend = run["backend"]
            label = _classify(backend, tag)
            if not label:
                continue
            n = frame_counts.get((model, backend, tag))
            if not n:
                print(f"[WARN] No override frame count for ({model}, {backend}, {tag}) — skipped")
                continue
            for host, host_data in run["hosts"].items():
                _record(model, label, host, host_data, n)

    return epf_map, debug_map
...
def main() -> None:
    ...
    print_summary(epf_map, debug_map)
    plot(epf_map)
