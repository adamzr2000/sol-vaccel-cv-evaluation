...
ISO_CPU_FILE = str(_HERE / "../../experiments/system-stats/summary-vaccel/iso_overall_cpu_stats_wifi.csv")  # robot local rows
...
def plot_combined_power(robot_rows, edge_cpu_rows, edge_gpu_rows):
    ...
    sns.set_theme(context="paper", style="ticks", rc={"xtick.direction": "out", "ytick.direction": "out", "font.family": "serif"}, font_scale=FONT_SCALE)
...
# Grab professional palettes
    ...
    for row_idx, (panel_key, ylabel, rows_data, vars_present) in enumerate(panels):
        ...
        for col_idx, current_models in enumerate(cat_models):
            ...
            ax.yaxis.set_major_locator(MaxNLocator(nbins=POWER_MAX_TICKS))
            ax.yaxis.set_major_formatter(FormatStrFormatter(POWER_TICK_FMT))
                    ...
                    if comps:
                        ...
                        if abs(savings_pct) > 0.5:
                            ...
                            ax.hlines(y=base_val, xmin=x_start, xmax=x_end, 
                                      colors=badge_color, linestyles='-.', linewidth=STROKE_WIDTH, alpha=0.75, zorder=4,
