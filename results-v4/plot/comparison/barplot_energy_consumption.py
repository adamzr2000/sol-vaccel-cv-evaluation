...
# all have a barplot_energy_consumption.py -- avoids import collisions.
vaccel_energy = _load_module(_VACCEL_DIR / "barplot_energy_consumption.py", "_vaccel_energy_source")
ros2_energy = _load_module(_ROS2_DIR / "barplot_energy_consumption.py", "_ros2_energy_source")

# Set the override on ros2_energy directly (a plain reassignment here would
# only rebind this file's own name, not ros2_energy.load_energy_map()'s).
# ROS2 only, Remote GPU only, applied uniformly to every semantic-
# segmentation model. Canonical definition lives in
# ros2/barplot_e2e_latency_and_fps.py (FPS/latency reads it too, and
# ros2_energy itself just aliases that same variable) -- set it there for
# it to take effect everywhere, or set ros2_energy's copy directly here for
# a run limited to this script.
#
# ros2_energy.SEG_REMOTE_GPU_TAG_OVERRIDE = "remote-gpu12fps"
...
def load_vaccel_energy_rows():
    ...
    with base_model/variant/mean/std keys, mean in kJ. Simpler than the
    per-frame loader: no perf-map / frame-count join needed."""
    ...
    robot_rows = vaccel_energy.load_robot_cpu_rows(cpu_df)
    edge_cpu_rows = vaccel_energy.load_edge_cpu_remote_rows(cpu_df)
    edge_gpu_rows = vaccel_energy.load_edge_gpu_remote_rows(gpu_df)
