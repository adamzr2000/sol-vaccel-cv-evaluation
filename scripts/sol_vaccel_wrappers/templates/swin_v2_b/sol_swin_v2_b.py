...
class sol_swin_v2_b:
    ...
    def init(self):
        ...
        self._session.exec_with_resource(
            resource,
            "sol_swin_v2_b_init_unpack",
        )

    def free(self):
        self._session.exec_with_resource(
            self._resource,
            "sol_swin_v2_b_free_unpack",
        )

    def sync(self):
        self._session.exec_with_resource(
            self._resource,
            "sol_swin_v2_b_sync_unpack",
        )
    ...
    def get_output(self):
        ...
        if out_resource is not None:
            with self.profiler("exec"):
                self._session.exec_with_resource(
                    self._resource,
                    "sol_swin_v2_b_get_output_unpack",
                )
