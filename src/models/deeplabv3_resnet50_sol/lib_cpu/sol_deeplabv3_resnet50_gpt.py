...
class sol_deeplabv3_resnet50:
    def open(self):
        ...
        self.init = self.lib.__getattr__("sol_deeplabv3_resnet50_init")
        self.init.restype = None
        self.init.argtypes = None

        self.free = self.lib.__getattr__("sol_deeplabv3_resnet50_free")
    ...
    def run(self, in__x = None, out__0_out = None, out__0_aux = None, vdims = None):
        ...
        # 2. Construct Argument List for C-Library
        # FIX: The compiled binary expects Outputs first, then Input
        call_args = [out__0_out, out__0_aux, in__x, vdims]
...
def run_example():
    ...
    # dp_args: Matches the Python wrapper signature (Input, Output...)
    dp_args = [in__x, out__0_out, out__0_aux, vdims] 

    # io_args: Matches the Binary C signature (Output, Input...)
    io_args = [out__0_out, out__0_aux, in__x, vdims]
    ...
    # Uses wrapper method signature
    lib.run(*dp_args)
    print(f"Max_V: {np.max(out__0_out, axis=1)}\nMax_I: {np.argmax(out__0_out, axis=1)}")
    print(f"Max_V: {np.max(out__0_aux, axis=1)}\nMax_I: {np.argmax(out__0_aux, axis=1)}")

    ####### Option 3: Run after setting in- and outputs #######
    # set_IO talks directly to C, so use binary order
    lib.set_IO(io_args)
