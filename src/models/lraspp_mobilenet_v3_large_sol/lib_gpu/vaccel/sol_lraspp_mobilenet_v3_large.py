...
class sol_lraspp_mobilenet_v3_large:
    def __init__(
        self,
        path=".",
        *,
    ...
    def init(self):
        ...
        self._session.exec_with_resource(
            resource,
            "sol_lraspp_mobilenet_v3_large_init_unpack",
        )

    def free(self):
        self._session.exec_with_resource(
            self._resource,
            "sol_lraspp_mobilenet_v3_large_free_unpack",
        )

    def sync(self):
        self._session.exec_with_resource(
            self._resource,
            "sol_lraspp_mobilenet_v3_large_sync_unpack",
        )
    ...
    def set_seed(self, seed):
        self._session.exec_with_resource(
            self._resource,
            "sol_lraspp_mobilenet_v3_large_set_seed_unpack",
    ...
    def optimize(self, level):
        self._session.exec_with_resource(
            self._resource,
            "sol_lraspp_mobilenet_v3_large_optimize_unpack",
            [vaccel.Arg(level, vaccel.ArgType.INT32)],
        )

    def _prepare_run_args(self, in__input, out__0_out=None, vdims=None):
        if vdims is None:
            vdims_0 = in__input.shape[0]
            vdims = np.array(
                [
                    vdims_0,
                ],
        ...
        return (in__input, out__0_out, vdims)

    def set_IO(self, args):
        (in__input, out__0_out, vdims) = self._prepare_run_args(*args)

        with self.profiler("init resources"):
            session = self._session
            in_resource = vaccel.Resource.from_numpy(in__input)
            out_resource = vaccel.Resource.from_numpy(out__0_out)
        ...
        with self.profiler("exec"):
            self._session.exec_with_resource(
                self._resource,
                "sol_lraspp_mobilenet_v3_large_set_IO_unpack",
                in_args,
            )

        self._arg_resources = [in_resource, out_resource, vdims_resource]

    def predict(self, in__input=None, out__0_out=None, vdims=None):
        if self._resource is None:
            self.init()

        (in__input, out__0_out, vdims) = self._prepare_run_args(
            in__input,
            out__0_out,
            vdims,
        )
        ...
        with self.profiler("init args"):
            in_args = [
                vaccel.Arg(in__input, vaccel.ArgType.BUFFER),
                vaccel.Arg(vdims, vaccel.ArgType.BUFFER),
            ]
            out_args = [vaccel.Arg(out__0_out, vaccel.ArgType.BUFFER)]
    ...
    def run(self, in__input=None, out__0_out=None, vdims=None):
        if in__input is not None:
            return self.predict(in__input, out__0_out, vdims)
        ...
        self._session.exec_with_resource(
            self._resource,
            "sol_lraspp_mobilenet_v3_large_run_unpack",
        )
        return None
    ...
    def get_output(self):
        ...
        if out_resource is not None:
            with self.profiler("exec"):
                self._session.exec_with_resource(
                    self._resource,
                    "sol_lraspp_mobilenet_v3_large_get_output_unpack",
                )
...
def run_example():
    ...
    vdims_0 = 1
    in__input = np.random.rand(vdims_0, 3, 224, 224).astype(np.float32)
    lib = sol_lraspp_mobilenet_v3_large()
    (out__0_out,) = lib(
        in__input,
    )
