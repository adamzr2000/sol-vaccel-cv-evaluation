...
VARIANTS = [
    ...
    "Edge-CPU (torch.compile)",           # Index 2
    "Edge-CPU (SOL)",                     # Index 3
    "Edge-GPU (torch.compile)",           # Index 4
    "Edge-GPU (SOL)",                     # Index 5
]
...
def hatch_for_variant_label(vlabel: str, row_idx: int) -> str | None:
    # Only apply hatching to the robot row (row 0)
    if row_idx != 0:
        return None
        
    s = vlabel.lower()
    if "edge-cpu" in s: return "///"
    if "edge-gpu" in s: return "...."
    return None


def compute_offsets(variants_present, row_idx: int):
    n = len(variants_present)
    # Use slim bars for row 0 (6 items), wider bars for the other rows (2 items)
...
def plot_combined_power(robot_rows, edge_cpu_rows, edge_gpu_rows):
    ...
    panels = [
        ("robot", "Robot CPU\npower (W)", robot_rows, VARIANTS),
        ("edge_cpu", "Edge CPU\npower (W)", edge_cpu_rows, [VARIANTS[2], VARIANTS[3]]),
        ("edge_gpu", "Edge GPU\npower (W)", edge_gpu_rows, [VARIANTS[4], VARIANTS[5]]),
    ]
        ...
        for col_idx, current_models in enumerate(cat_models):
            ...
            for v in vars_present:
                ...
                ax.bar(
                    ...
                    hatch=hatch_for_variant_label(v, row_idx), # Pass row_idx here
                ...
                if SHOW_ERROR_BARS:
                    ...
                    if np.any(yerr > 0):
                        ax.errorbar(
                            ...
                            elinewidth=1.0, capsize=4, capthick=1.0, zorder=10 # capsize=4 restores horizontal ends
                        )
    ...
    # Global Legend (from Row 0, Col 0 which has all 6 variants)
