...
def plot_host(ax, dfh: pd.DataFrame, host: str, base_models, color_map, y_lim_top):
    ...
    for _, r in dfh.iterrows():
        ...
        if (m in base_models) and (vv in variants):
            mean_map[(m, vv)] = float(r["mem_mb_mean"])
            std_map[(m, vv)] = float(r["mem_mb_std"]) if pd.notna(r["mem_mb_std"]) else np.nan
...
def main():
    ...
    if PLOT_MODE == "combined":
        ...
        for ax, host in zip(axes, present_hosts):
            ...
            ax.set_ylabel(f"{pretty_host_name(host)} RAM utilization (MB)")
