...
int main(int argc, char** argv) {
	...
	sol_f32 *_input_0 = (sol_f32*) malloc(vdims_0 * 3ll * 224ll * 224ll *  sizeof(sol_f32));
	
	// Define and Initialize Outputs---------------------------------------------
	sol_f32 *_0 = (sol_f32*) malloc(vdims_0 * 1000ll *  sizeof(sol_f32));
	...
	for(size_t n = 0; n < vdims_0 * 1000ll * 1; n++)
		_0[n] = 0;
	
	// Call generated library---------------------------------------------
	sol_mobilenet_v3_large_init(); // optional, reads parameters and moves them to device if necessary
	sol_mobilenet_v3_large_set_seed(314159); // optional
	...
	// Output #0
	data_per_batch = (vdims_0 * 1000ll * 1)/vdims_0;
	...
	sol_mobilenet_v3_large_free(); // frees allocated parameters by lib
