...
class sol_deeplabv3_resnet50:
        ...
        def run(self, in__x = None, out__0_out = None, vdims = None):
                ...
                if out__0_out is None:
                        out__0_out = np.zeros((1, 21, 224, 224), dtype=np.float32)
        ...
        def close(self):
                dlclose_func = ctypes.CDLL(self.path + "/" + "libdeeplabv3_resnet50.so").dlclose
...
def run_example():
        ...
        in__x = np.random.rand(1, 3, 224, 224).astype(np.float32)
        lib = sol_deeplabv3_resnet50()
        out__0_out,  = lib(in__x, )
        ...
        in__x = np.random.rand(1, 3, 224, 224).astype(np.float32)
        out__0_out = np.zeros((1, 21, 224, 224), dtype=np.float32)
        dp_args = [in__x, out__0_out, vdims] # Inputs, Outputs, VDims must be in this exact order!
