...
/*
 * sol_predict()/sol_*_set_IO() are generated by the SOL compiler with a
 * fixed (in, out, aux, vdims) signature that we cannot change. The aux
 * (auxiliary classifier) head is never consumed by callers, so instead of
 * registering it as a client-visible vaccel resource (which would ship it
 * back over vaccel-remote RPC), we hand the compiled kernel a reusable
 * local scratch buffer. This is the only reason these _unpack functions
 * differ from the SOL-generated original; reverting is a straight diff
 * revert if a future SOL export drops the aux head on its own.
 */
static void *aux_scratch_alloc(void **buf, size_t *cap, size_t need)
{
	if (*cap < need) {
		       ...
		       struct vaccel_arg *write, size_t nr_write)
{
	...
	static void *aux_scratch;
	static size_t aux_scratch_size;
	sol_f32 *out__0_aux = (sol_f32 *)aux_scratch_alloc(&aux_scratch,
							    &aux_scratch_size,
							    size);
	if (out__0_aux == NULL) {
		vaccel_prof_region_stop(&predict_unpack_stats);
		sol_error(func, "Failed to allocate aux scratch buffer");
	 ...
	 * before this function returns and the RPC framework marshals
	 * out__0_out into the response, so the client always receives valid,
				...
				struct vaccel_arg *write, size_t nr_write)
{
	...
	static void *aux_scratch_io;
	static size_t aux_scratch_io_size;
	sol_f32 *out__0_aux = (sol_f32 *)aux_scratch_alloc(
		&aux_scratch_io, &aux_scratch_io_size,
		out_out_res->blobs[0]->size);
	if (out__0_aux == NULL) {
		vaccel_prof_region_stop(&set_io_unpack_stats);
		sol_error(func, "Failed to allocate aux scratch buffer");
		return VACCEL_ENOMEM;
