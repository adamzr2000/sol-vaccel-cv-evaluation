...
class sol_swin3d_b:
	def open(self):
		self.lib = ctypes.CDLL(self.path + "/" + "libsol_swin3d_b.so")

		self.call = self.lib.__getattr__("sol_predict") 
		self.call.restype = None

		self.init = self.lib.__getattr__("sol_swin3d_b_init")
		self.init.restype = None
		self.init.argtypes = None

		self.free = self.lib.__getattr__("sol_swin3d_b_free")
		self.free.restype = None
		self.free.argtypes = None

		self.seed_ = self.lib.__getattr__("sol_swin3d_b_set_seed")
		self.seed_.argtypes = [ctypes.c_uint64]
		self.seed_.restype = None

		self.set_IO_ = self.lib.__getattr__("sol_swin3d_b_set_IO")
		self.set_IO_.restype = None

		self.call_no_args = self.lib.__getattr__("sol_swin3d_b_run")
		self.call_no_args.argtypes = None
		self.call_no_args.restype = None

		self.get_output = self.lib.__getattr__("sol_swin3d_b_get_output")
		self.get_output.argtypes = None
		self.get_output.restype = None

		self.sync = self.lib.__getattr__("sol_swin3d_b_sync")
		self.sync.argtypes = None
		self.sync.restype = None

		self.opt_ = self.lib.__getattr__("sol_swin3d_b_optimize")
	...
	def run(self, in__input_0 = None, out__0 = None, vdims = None):
		...
		if out__0 is None:
			out__0 = np.zeros((1, 400), dtype=np.float32)
	...
	def close(self):
		dlclose_func = ctypes.CDLL(self.path + "/" + "libsol_swin3d_b.so").dlclose
...
def run_example():
	...
	in__input_0 = np.random.rand(1, 3, 16, 112, 112).astype(np.float32)
	lib = sol_swin3d_b()
