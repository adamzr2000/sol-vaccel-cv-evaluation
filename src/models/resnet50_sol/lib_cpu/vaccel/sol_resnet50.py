...
class sol_resnet50:
    ...
    def init(self):
        ...
        self._session.exec_with_resource(
            resource,
            "sol_resnet50_init_unpack",
        )

    def free(self):
        self._session.exec_with_resource(
            self._resource,
            "sol_resnet50_free_unpack",
        )

    def sync(self):
        self._session.exec_with_resource(
            self._resource,
            "sol_resnet50_sync_unpack",
        )
    ...
    def get_output(self):
        ...
        if out_resource is not None:
            with self.profiler("exec"):
                self._session.exec_with_resource(
                    self._resource,
                    "sol_resnet50_get_output_unpack",
                )
