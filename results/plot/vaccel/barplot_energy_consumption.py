...
Energy = cpu_energy_j_total / gpu_energy_j_total columns - exact hardware-
counter totals (RAPL for CPU, NVML for GPU) computed in
summarize_system_stats.py from the collector's energy_totals sidecar, not a
mean(power) x duration approximation - minus the corresponding idle baseline
(idle/summary.csv's exact counter power, scaled to this run's own
duration_sec), so bars show energy attributable to the workload rather than
including the host's always-on idle draw.
...
from plot_config import get_path, load_config, get_model_type_order, get_model_display_name, apply_seg_remote_override
...
def _safe_energy_cpu(row, idle_power_w: float | None = None) -> float:
    """Exact CPU energy (kJ) from cpu_energy_j_total (RAPL hardware-counter
    total) minus the idle baseline for this run's duration, not the
...
def _safe_energy_gpu(row, idle_power_w: float | None = None) -> float:
    """Exact GPU energy (kJ) from gpu_energy_j_total (NVML hardware-counter
    total) minus the idle baseline for this run's duration, not the
    mean(power) x duration approximation."""
    try:
...
def load_robot_cpu_rows(cpu_df: pd.DataFrame, idle_power_w: float | None):
    ...
    for _, r in sub.iterrows():
        ...
        if v is None:
            continue
        energy = _safe_energy_cpu(r, idle_power_w)
        rows.append({
...
def load_edge_cpu_remote_rows(cpu_df: pd.DataFrame, idle_power_w: float | None):
    rows = []
...
def load_edge_gpu_remote_rows(gpu_df: pd.DataFrame, idle_power_w: float | None):
    rows = []
    for _, r in gpu_df.iterrows():
        ...
        if "edge" not in host or "gpu" not in device:
            continue
        model  = str(r.get("model", "")).strip()
        energy = _safe_energy_gpu(r, idle_power_w)
...
def main():
    ...
    robot_rows    = load_robot_cpu_rows(cpu_df, robot_idle_w)
    edge_cpu_rows = load_edge_cpu_remote_rows(cpu_df, edge_cpu_idle_w)
    edge_gpu_rows = load_edge_gpu_remote_rows(gpu_df, edge_gpu_idle_w)
