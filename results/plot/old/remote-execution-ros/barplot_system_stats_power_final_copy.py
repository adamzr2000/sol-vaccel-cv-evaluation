        ...
        for col_idx, current_models in enumerate(cat_models):
            ...
            if idle_key in IDLE_POWER:
                ...
                ax.hlines(y=idle_mean, xmin=x_min, xmax=x_max, 
                         colors='gray', linestyles=':', linewidth=2.0, alpha=0.6, zorder=2,
                         label="Idle" if (row_idx == 0 and col_idx == 0) else "")

            # --- START: POWER SAVINGS INDICATOR ---
            # Row 0 (Robot): Generic offloading benefit (local avg → remote avg)
            # Row 1 (Edge CPU): SOL optimization benefit (torch → SOL)
            # Row 2 (Edge GPU): SOL optimization benefit (torch → SOL)
            for i, m in enumerate(current_models):
                ...
                if row_idx == 0:
                    # Row 0: Generic offloading benefit = avg(Local torch, Local SOL) vs avg(All remotes)
                    local_vals = [
                        mean_map.get((m, v), np.nan)
                        for v in local_base_vars
                        if np.isfinite(mean_map.get((m, v), np.nan)) and mean_map.get((m, v), np.nan) > 0
                    ]
                    if len(local_vals) < 2:
                        continue
                    base_val = np.mean(local_vals)  # Average of both local variants
...
# 2. Calculate average of the target variants
    ...
    variant_labels = list(VARIANTS) + ["Idle power", "Energy benefit"]
