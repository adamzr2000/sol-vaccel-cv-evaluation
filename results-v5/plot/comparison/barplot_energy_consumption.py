#!/usr/bin/env python3
"""
barplot_energy_consumption.py (comparison, paper-facing)

Unified vAccel-vs-ROS2 total energy consumption comparison (kJ), following
the exact same format as barplot_energy_per_frame.py -- same 3-row layout,
same 3-hue x 4-shade teal/orange/purple palette, same zoom inset and
head-to-head connector on Row 0. Simpler than the per-frame version only in
the data loading: no frame-count join is needed (energy is just
mean_power_W x duration_sec, or the pre-computed gpu_energy_j column), so
there's no perf-map plumbing on the vAccel side and no frame_counts arg on
the ROS2 side.

Color: hue = deployment scenario (teal=Local CPU, orange/magenta=Remote CPU,
purple=Remote GPU). Within each hue, 4 shades cover framework x backend:
[ROS2+Torch, ROS2+SOL, vAccel+Torch, vAccSOL], validated against
scripts/validate_palette.py (dataviz skill) to clear the normal-vision
adjacency floor (teal 18.2, orange/magenta 19.4, purple 16.4).

Row structure (not three symmetric scenario rows -- three energy domains
that don't all apply to every scenario):
  Row 0 - Robot CPU (kJ): present in all 3 scenarios -- 3 scenario-groups x
          4 bars, plus a zoom inset (Remote CPU/GPU dwarfed by Local CPU on
          the main axis) and a ROS2-vs-vAccel connector on Local CPU.
  Row 1 - Edge CPU (kJ): Remote CPU scenario only -- 4 bars + connector.
  Row 2 - Edge GPU (kJ): Remote GPU scenario only -- 4 bars + connector.

Not carried over from the source scripts: the per-model Torch->SOL savings
badges (arrows + % labels), same decision as the per-frame comparison.

Produces: energy-consumption-comparison.pdf
"""
...
# Loaded via import_from (not sys.path/plain import) since vaccel/, ros2/,
# and this file all have same-named files (plot_config.py,
# barplot_energy_consumption.py) - a plain import would cache whichever
# loads first under sys.modules and silently feed it to the other
# directory's same-named internal imports. See _dual_import.py.
vaccel_plot_config = import_from(_VACCEL_DIR, "plot_config.py", "vaccel_plot_config")
vaccel_data = import_from(_VACCEL_DIR, "plot_data_e2e.py", "vaccel_data")  # shared CATEGORIES/model order
get_model_display_name = vaccel_plot_config.get_model_display_name

vaccel_energy = import_from(_VACCEL_DIR, "barplot_energy_consumption.py", "vaccel_energy")

# Semantic-segmentation models only, remote-{cpu,gpu} cases only: energy is
# read from ros2/plot_config.py's SEG_REMOTE_CPU_TAG_OVERRIDE /
# SEG_REMOTE_GPU_TAG_OVERRIDE (e.g. a 12fps-capped rerun) - ros2_energy
# inherits whatever's configured there automatically, same as
# ros2/barplot_energy_consumption.py itself.
ros2_energy = import_from(_ROS2_DIR, "barplot_energy_consumption.py", "ros2_energy")

OUTPUT_FILE = "energy-consumption-comparison.pdf"
...
# Same palette as barplot_energy_per_frame.py -- teal=Local CPU,
# orange/magenta=Remote CPU, purple=Remote GPU, 4 shades each (middle two
# steps swapped from a plain ramp to widen the adjacent-bar gap). Validated
...
def load_vaccel_energy_rows():
    """Replicates vaccel/barplot_energy_consumption.py's main() data path,
    returning (robot_rows, edge_cpu_rows, edge_gpu_rows) -- lists of dicts
    with base_model/variant/mean/std keys, mean in kJ (exact hardware-counter
    totals, idle-baseline subtracted, segmentation-remote run-tag override
    applied) -- replicates vaccel/barplot_energy_consumption.py's main()
    data path exactly, using its own current functions rather than a
    hand-copied one, so this stays in sync automatically as that script
    changes."""
    cpu_path = Path(vaccel_energy.CPU_FILE).resolve()
    iso_cpu_path = Path(vaccel_energy.ISO_CPU_FILE).resolve()
    gpu_path = Path(vaccel_energy.GPU_FILE).resolve()
    ...
    cpu_df = vaccel_energy.apply_seg_remote_override(cpu_df, "cpu")
    gpu_df = vaccel_energy.apply_seg_remote_override(gpu_df, "gpu")

    if vaccel_energy.WORKLOAD_ONLY:
        idle_power = vaccel_energy.load_idle_power_map(vaccel_energy.IDLE_SUMMARY_FILE)
        robot_idle_w = idle_power.get(vaccel_energy.IDLE_HOST_ROBOT)
        edge_cpu_idle_w = idle_power.get(vaccel_energy.IDLE_HOST_EDGE_CPU)
        edge_gpu_idle_w = idle_power.get(vaccel_energy.IDLE_HOST_EDGE_GPU)
    else:
        robot_idle_w = edge_cpu_idle_w = edge_gpu_idle_w = None

    robot_rows = vaccel_energy.load_robot_cpu_rows(cpu_df, robot_idle_w)
    edge_cpu_rows = vaccel_energy.load_edge_cpu_remote_rows(cpu_df, edge_cpu_idle_w)
    edge_gpu_rows = vaccel_energy.load_edge_gpu_remote_rows(gpu_df, edge_gpu_idle_w)

    robot_rows = vaccel_energy._apply_model_filter(robot_rows, ALLOWED_MODELS)
    edge_cpu_rows = vaccel_energy._apply_model_filter(edge_cpu_rows, ALLOWED_MODELS)
    edge_gpu_rows = vaccel_energy._apply_model_filter(edge_gpu_rows, ALLOWED_MODELS)
...
def load_merged_maps():
    """Returns three dicts:
      robot_cpu_map: (model, scenario, fw, be) -> kJ
      edge_cpu_map:  (model, fw, be) -> kJ   (Remote CPU scenario only)
      edge_gpu_map:  (model, fw, be) -> kJ   (Remote GPU scenario only)
    """
    robot_cpu_map, edge_cpu_map, edge_gpu_map = {}, {}, {}

    robot_rows, edge_cpu_rows, edge_gpu_rows = load_vaccel_energy_rows()
    for r in robot_rows:
    ...
    ros2_map = ros2_energy.load_energy_map()
    for (model, variant, host, kind), e_kj in ros2_map.items():
        if model not in ALLOWED_MODELS:
            continue
        scenario, backend = scenario_backend_from_variant(variant)
        if host == "robot" and kind == "cpu":
            robot_cpu_map[(model, scenario, "ROS2", backend)] = e_kj
        elif host == "edge-asus" and kind == "cpu" and scenario == "Remote CPU":
            edge_cpu_map[(model, "ROS2", backend)] = e_kj
        elif host == "edge-asus" and kind == "gpu" and scenario == "Remote GPU":
            edge_gpu_map[(model, "ROS2", backend)] = e_kj

    return robot_cpu_map, edge_cpu_map, edge_gpu_map


def print_debug_info(robot_cpu_map, edge_cpu_map, edge_gpu_map):
    """Prints every plotted value (kJ) to the console, same layout as the
    figure: Robot CPU per scenario, then Edge CPU (Remote CPU only), then
    Edge GPU (Remote GPU only)."""
    present = sorted({m for (m, *_rest) in robot_cpu_map})
    base_models = vaccel_data.ordered_models(present)

    W = 108
    print("\n" + "=" * W)
    print(f"{'DEBUG: PLOTTED ENERGY CONSUMPTION (kJ)':^{W}}")
    print("=" * W)
    header = (f"{'Model':<22} | {'Row':<10} | {'Scenario':<11} | "
              f"{'ROS2+Torch':>10} | {'ROS2+SOL':>10} | {'vAccel+Torch':>12} | {'vAccSOL':>10}")
    print(header)
    print("-" * W)

    def fmt_row(model, row_label, scenario, data_map, key_fn):
        vals = []
        for fw, be in GROUP_ORDER:
            v = data_map.get(key_fn(model, fw, be), np.nan)
            vals.append(f"{v:10.3f}" if np.isfinite(v) else f"{'-':>10}")
        print(f"{model:<22} | {row_label:<10} | {scenario:<11} | "
              f"{vals[0]} | {vals[1]:>10} | {vals[2]:>12} | {vals[3]:>10}")

    for m in base_models:
        for scenario in SCENARIOS:
            fmt_row(m, "Robot CPU", scenario, robot_cpu_map, lambda mo, fw, be, s=scenario: (mo, s, fw, be))
        fmt_row(m, "Edge CPU", "Remote CPU", edge_cpu_map, lambda mo, fw, be: (mo, fw, be))
        fmt_row(m, "Edge GPU", "Remote GPU", edge_gpu_map, lambda mo, fw, be: (mo, fw, be))
        print("-" * W)
    print("=" * W + "\n")
...
def plot_combined(robot_cpu_map, edge_cpu_map, edge_gpu_map):
    ...
    for col_idx, current_models in enumerate(cat_models):
        ...
        ax0.margins(x=0.015)
        if col_idx == 0:
            ax0.set_ylabel("Robot CPU (kJ)")

        # ================= ROWS 1-2: Edge CPU / Edge GPU =====================
        row_specs = [
            (1, "Remote CPU", edge_cpu_map, "Edge CPU (kJ)"),
            (2, "Remote GPU", edge_gpu_map, "Edge GPU (kJ)"),
        ]
...
def main():
    ...
    print_debug_info(robot_cpu_map, edge_cpu_map, edge_gpu_map)
