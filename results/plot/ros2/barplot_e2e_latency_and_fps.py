...
from plot_config import SEG_MODELS, SEG_REMOTE_CPU_TAG_OVERRIDE, SEG_REMOTE_GPU_TAG_OVERRIDE
...
# Semantic-segmentation models only, remote-{cpu,gpu} cases only: FPS/latency
# is read from SEG_REMOTE_CPU_TAG_OVERRIDE / SEG_REMOTE_GPU_TAG_OVERRIDE
# (plot_config.py) instead of the default remote-cpu/remote-gpu_model_stats.json.
# Single source of truth: plot_config.py - barplot_energy_consumption.py and
# barplot_energy_per_frame.py import the exact same variables, since
# FPS/latency and energy come from the same underlying benchmark run and
# must stay in sync.
...
def load_rows() -> list[tuple]:
    ...
    for tag, override_tag in seg_overrides.items():
        if not override_tag:
            continue
        ov_path = SUMMARY_DIR / f"{override_tag}_model_stats.json"
        if not ov_path.exists():
            print(f"[WARN] Missing override model-stats: {ov_path}")
            continue
        for r in json.loads(ov_path.read_text())["runs"]:
            if r.get("model", "") not in SEG_MODELS:
                continue
            variant = _classify_as(r, tag)
            if variant is None:
                continue
            row = _row_from_run(r, variant)
            if row is not None:
