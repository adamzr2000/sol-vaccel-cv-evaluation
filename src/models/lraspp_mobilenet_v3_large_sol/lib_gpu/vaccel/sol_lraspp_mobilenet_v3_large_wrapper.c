#include "sol_lraspp_mobilenet_v3_large.h"
#include <assert.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdint.h>
...
int sol_lraspp_mobilenet_v3_large_init_unpack(struct vaccel_arg *read, size_t nr_read,
			      struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_init();
		       ...
		       struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_f32 *in__input;
	size_t size;
	ret = vaccel_arg_array_get_buffer(&read_args, (void **)&in__input,
					  &size);
	if (ret) {
		vaccel_prof_region_stop(&predict_unpack_stats);
	...
	sol_predict(in__input, out__0_out, vdims);
	/*
	 ...
	 */
	sol_lraspp_mobilenet_v3_large_sync();
...
int sol_lraspp_mobilenet_v3_large_set_IO_unpack(struct vaccel_arg *read, size_t nr_read,
				struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_f32 *in__input = (sol_f32 *)in_res->blobs[0]->data;
	sol_f32 *out__0_out = (sol_f32 *)out_res->blobs[0]->data;
	sol_s64 *vdims = (sol_s64 *)vdims_res->blobs[0]->data;

	vaccel_prof_region_stop(&set_io_unpack_stats);

	vaccel_prof_region_start(&set_io_stats);
	sol_lraspp_mobilenet_v3_large_set_IO(in__input, out__0_out, vdims);
...
int sol_lraspp_mobilenet_v3_large_run_unpack(struct vaccel_arg *read, size_t nr_read,
			     struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_run();
...
int sol_lraspp_mobilenet_v3_large_optimize_unpack(struct vaccel_arg *read, size_t nr_read,
				  struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_optimize(level);
...
int sol_lraspp_mobilenet_v3_large_sync_unpack(struct vaccel_arg *read, size_t nr_read,
			      struct vaccel_arg *write, size_t nr_write)
{
...
int sol_lraspp_mobilenet_v3_large_get_output_unpack(struct vaccel_arg *read, size_t nr_read,
				    struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_get_output();
...
int sol_lraspp_mobilenet_v3_large_free_unpack(struct vaccel_arg *read, size_t nr_read,
			      struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_free();
...
int sol_lraspp_mobilenet_v3_large_free_host_unpack(struct vaccel_arg *read, size_t nr_read,
				   struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_free_host();
...
int sol_lraspp_mobilenet_v3_large_free_device_unpack(struct vaccel_arg *read, size_t nr_read,
				     struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_free_device();
...
int sol_lraspp_mobilenet_v3_large_set_seed_unpack(struct vaccel_arg *read, size_t nr_read,
				  struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_lraspp_mobilenet_v3_large_set_seed(seed);
