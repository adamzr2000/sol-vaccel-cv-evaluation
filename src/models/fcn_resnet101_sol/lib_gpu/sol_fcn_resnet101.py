...
class sol_fcn_resnet101:
	def open(self):
		self.lib = ctypes.CDLL(self.path + "/" + "libsol_fcn_resnet101.so")

		self.call = self.lib.__getattr__("sol_predict") 
		self.call.restype = None

		self.init = self.lib.__getattr__("sol_fcn_resnet101_init")
		self.init.restype = None
		self.init.argtypes = None

		self.free = self.lib.__getattr__("sol_fcn_resnet101_free")
		self.free.restype = None
		self.free.argtypes = None

		self.seed_ = self.lib.__getattr__("sol_fcn_resnet101_set_seed")
		self.seed_.argtypes = [ctypes.c_uint64]
		self.seed_.restype = None

		self.set_IO_ = self.lib.__getattr__("sol_fcn_resnet101_set_IO")
		self.set_IO_.restype = None

		self.call_no_args = self.lib.__getattr__("sol_fcn_resnet101_run")
		self.call_no_args.argtypes = None
		self.call_no_args.restype = None

		self.get_output = self.lib.__getattr__("sol_fcn_resnet101_get_output")
		self.get_output.argtypes = None
		self.get_output.restype = None

		self.sync = self.lib.__getattr__("sol_fcn_resnet101_sync")
		self.sync.argtypes = None
		self.sync.restype = None

		self.opt_ = self.lib.__getattr__("sol_fcn_resnet101_optimize")
	...
	def close(self):
		dlclose_func = ctypes.CDLL(self.path + "/" + "libsol_fcn_resnet101.so").dlclose
