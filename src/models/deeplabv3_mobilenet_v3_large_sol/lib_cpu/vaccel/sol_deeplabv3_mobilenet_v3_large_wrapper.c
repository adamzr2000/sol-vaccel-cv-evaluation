#include "sol_deeplabv3_mobilenet_v3_large.h"
#include <assert.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdint.h>
...
int sol_deeplabv3_mobilenet_v3_large_init_unpack(struct vaccel_arg *read, size_t nr_read,
			      struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_init();
		       ...
		       struct vaccel_arg *write, size_t nr_write)
{
	...
	int ret = check_nr_args(2, 2, nr_read, nr_write, func);
	if (ret)
		return ret;
	...
	sol_f32 *in__x;
	size_t size;
	ret = vaccel_arg_array_get_buffer(&read_args, (void **)&in__x,
					  &size);
	if (ret) {
		vaccel_prof_region_stop(&predict_unpack_stats);
	...
	sol_f32 *out__0_out;
	ret = vaccel_arg_array_get_buffer(&write_args, (void **)&out__0_out, &size);
	if (ret) {
		vaccel_prof_region_stop(&predict_unpack_stats);
		sol_error(func, "Failed to unpack output out");
		return ret;
	}

	sol_f32 *out__0_aux;
	ret = vaccel_arg_array_get_buffer(&write_args, (void **)&out__0_aux, &size);
	if (ret) {
		vaccel_prof_region_stop(&predict_unpack_stats);
		sol_error(func, "Failed to unpack output aux");
		return ret;
	}
	...
	sol_predict(in__x, out__0_out, out__0_aux, vdims);
...
int sol_deeplabv3_mobilenet_v3_large_set_IO_unpack(struct vaccel_arg *read, size_t nr_read,
				struct vaccel_arg *write, size_t nr_write)
{
	const char *func = "set_IO";
	int ret = check_nr_args(5, 0, nr_read, nr_write, func);
	if (ret)
		return ret;
	...
	vaccel_id_t out_out_res_id;
	ret = vaccel_arg_array_get_int64(&read_args, &out_out_res_id);
	if (ret) {
		vaccel_prof_region_stop(&set_io_unpack_stats);
		sol_error(func, "Failed to unpack output out resource ID");
		return ret;
	}

	vaccel_id_t out_aux_res_id;
	ret = vaccel_arg_array_get_int64(&read_args, &out_aux_res_id);
	if (ret) {
		vaccel_prof_region_stop(&set_io_unpack_stats);
		sol_error(func, "Failed to unpack output aux resource ID");
		return ret;
	}
	...
	struct vaccel_resource *out_out_res;
	ret = vaccel_session_resource_by_id(sess, &out_out_res, out_out_res_id);
	if (ret) {
		vaccel_prof_region_stop(&set_io_unpack_stats);
		sol_error(func, "Unknown resource %" PRId64, out_out_res_id);
		return ret;
	}

	struct vaccel_resource *out_aux_res;
	ret = vaccel_session_resource_by_id(sess, &out_aux_res, out_aux_res_id);
	if (ret) {
		vaccel_prof_region_stop(&set_io_unpack_stats);
		sol_error(func, "Unknown resource %" PRId64, out_aux_res_id);
		return ret;
	}
	...
	sol_f32 *in__x = (sol_f32 *)in_res->blobs[0]->data;
	sol_f32 *out__0_out = (sol_f32 *)out_out_res->blobs[0]->data;
	sol_f32 *out__0_aux = (sol_f32 *)out_aux_res->blobs[0]->data;
	sol_s64 *vdims = (sol_s64 *)vdims_res->blobs[0]->data;

	vaccel_prof_region_stop(&set_io_unpack_stats);

	vaccel_prof_region_start(&set_io_stats);
	sol_deeplabv3_mobilenet_v3_large_set_IO(in__x, out__0_out, out__0_aux, vdims);
...
int sol_deeplabv3_mobilenet_v3_large_run_unpack(struct vaccel_arg *read, size_t nr_read,
			     struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_run();
...
int sol_deeplabv3_mobilenet_v3_large_optimize_unpack(struct vaccel_arg *read, size_t nr_read,
				  struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_optimize(level);
...
int sol_deeplabv3_mobilenet_v3_large_sync_unpack(struct vaccel_arg *read, size_t nr_read,
			      struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_sync();
...
int sol_deeplabv3_mobilenet_v3_large_get_output_unpack(struct vaccel_arg *read, size_t nr_read,
				    struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_get_output();
...
int sol_deeplabv3_mobilenet_v3_large_free_unpack(struct vaccel_arg *read, size_t nr_read,
			      struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_free();
...
int sol_deeplabv3_mobilenet_v3_large_free_host_unpack(struct vaccel_arg *read, size_t nr_read,
				   struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_free_host();
...
int sol_deeplabv3_mobilenet_v3_large_free_device_unpack(struct vaccel_arg *read, size_t nr_read,
				     struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_free_device();
...
int sol_deeplabv3_mobilenet_v3_large_set_seed_unpack(struct vaccel_arg *read, size_t nr_read,
				  struct vaccel_arg *write, size_t nr_write)
{
	...
	sol_deeplabv3_mobilenet_v3_large_set_seed(seed);
