...
def load_energy_totals(csv_path: Path) -> Optional[dict]:
    ...
    energy_totals_<stem>.json sidecar, written next to the CSV.

    Tries the current naming (exact, unstripped CSV stem) first, then falls
    back to an older collector naming convention that stripped a trailing
    "_cpu"/"_gpu" device suffix, so runs captured before that was reverted
    are still picked up. Returns None if neither exists - callers should
    ...
    for cand in candidates:
        totals_path = csv_path.parent / f"energy_totals_{cand}.json"
...
def main() -> None:
    ...
    ap.add_argument("--link", choices=["5g", "wifi"], default="wifi")
