...
3-row x 3-column figure:
  Row 0 - Robot CPU energy (6 variants: local-cpu, remote-cpu, remote-gpu x Torch/SOL)
  Row 1 - Edge CPU energy during remote-CPU inference (Torch + SOL)
  Row 2 - Edge GPU energy during remote-GPU inference (Torch + SOL)

energy_kJ = (cpu_energy_j_total / gpu_energy_j_total - idle_power_w * duration_sec) / 1000
  - cpu/gpu_energy_j_total: exact hardware-counter totals (RAPL/NVML) from
    system-stats _summary JSONs, not a mean(power) x duration approximation.
  - idle_power_w: exact idle baseline from ../../experiments/system-stats/idle/summary.csv
    (see plot_config.py), scaled to this run's own duration_sec, so bars show
    energy attributable to the workload rather than the host's always-on draw.
...
Run from results/plot/:
  python3 barplot_energy_consumption.py
"""
...
from plot_config import (
    SEG_MODELS, SEG_REMOTE_CPU_TAG_OVERRIDE, SEG_REMOTE_GPU_TAG_OVERRIDE,
    load_idle_power_map, idle_host_for, subtract_idle,
)
...
ROW_CONFIGS = [
    (0, VARIANTS,     "Robot CPU \n(kJ)",     "robot",     "cpu", ROBOT_BAR_W),
    (1, CPU_VARIANTS, "Edge CPU \n(kJ)", "edge-asus", "cpu", EDGE_BAR_W),
    (2, GPU_VARIANTS, "Edge GPU \n(kJ)", "edge-asus", "gpu", EDGE_BAR_W),
]
...
def load_energy_map() -> dict[tuple, float]:
    """
    Returns {(model, variant_label, host, kind): energy_kJ}, using the exact
    hardware-counter total (cpu_energy_j_total / gpu_energy_j_total) minus
    the corresponding idle baseline for that run's own duration - see
    plot_config.py's idle_host_for()/subtract_idle().
    """
    energy_map: dict[tuple, float] = {}
    ...
    def _record(model, label, host, host_data):
        for kind, e_key in [("cpu", "cpu_energy_j_total"), ("gpu", "gpu_energy_j_total")]:
    ...
    for tag in ("local-cpu", "remote-cpu", "remote-gpu"):
        ...
        for run in json.loads(path.read_text())["runs"]:
            model   = run["model"]
            backend = run["backend"]
            # Segmentation models read their Remote CPU/GPU rows from the
            # override file below instead, when one is configured.
            if tag in seg_overrides and model in SEG_MODELS and seg_overrides[tag]:
                continue
            rt      = run["run_tag"]
            ...
            for host, host_data in run["hosts"].items():
                _record(model, label, host, host_data)

    # Segmentation Remote CPU/GPU override: pull every segmentation model's
    # rows from the configured summary file instead, classified as the
    # normal "Remote CPU"/"Remote GPU" variant regardless of that file's own
    # run_tag (e.g. a "remote-gpu12fps"-tagged rerun still reads as "Remote GPU").
    for tag, override_tag in seg_overrides.items():
        if not override_tag:
            continue
        ov_path = STATS_DIR / f"{override_tag}_system_stats.json"
        if not ov_path.exists():
            print(f"[WARN] Missing override system-stats: {ov_path}")
            continue
        for run in json.loads(ov_path.read_text())["runs"]:
            if run["model"] not in SEG_MODELS:
                continue
            label = _classify(run["backend"], tag)
            if not label:
                continue
            for host, host_data in run["hosts"].items():
                _record(run["model"], label, host, host_data)
...
def plot(energy_map: dict[tuple, float]) -> None:
    all_models = _ordered([m for (m, *_) in energy_map if m in MODEL_ORDER])
    if not all_models:
    ...
    for r_idx, v_set, _, host_f, kind_f, _ in ROW_CONFIGS:
        for col_idx, current_models in enumerate(cat_models):
            vals = np.array([
                energy_map.get((m, v, host_f, kind_f), np.nan)
    ...
    for r_idx, v_set, ylabel, host_f, kind_f, bar_w in ROW_CONFIGS:
        mean_map = {(m, v): energy_map.get((m, v, host_f, kind_f), np.nan)
...
def print_summary(energy_map: dict[tuple, float]) -> None:
    print(f"\n{'model':26} {'variant':36} {'host':12} {'kind':4} {'kJ':>10}")
    print("-" * 95)
    for (model, variant, host, kind), e in sorted(
        energy_map.items(), key=lambda kv: (kv[0][1], kv[0][0])
    ):
        print(f"{model:26} {variant:36} {host:12} {kind:4} {e:10.4f}")
    print()
