...
# barplot_energy_per_frame.py) - a plain import would cache whichever loads
# first under sys.modules and silently feed it to the other directory's
# same-named internal imports. See _dual_import.py.
...
vaccel_epf = import_from(_VACCEL_DIR, "barplot_energy_per_frame.py", "vaccel_epf")

# Semantic-segmentation models only, remote-{cpu,gpu} cases only: energy and
# frame counts are read from ros2/plot_config.py's SEG_REMOTE_CPU_TAG_OVERRIDE
# / SEG_REMOTE_GPU_TAG_OVERRIDE (e.g. a 12fps-capped rerun) - ros2_epf
# inherits whatever's configured there automatically, same as
# ros2/barplot_energy_per_frame.py itself.
ros2_epf = import_from(_ROS2_DIR, "barplot_energy_per_frame.py", "ros2_epf")
...
def load_vaccel_epf_rows():
    ...
    # Replicates vaccel/barplot_energy_per_frame.py's main() data path
    # exactly (seg-remote-run-tag override, idle-baseline subtraction),
    # using its own current functions rather than a hand-copied one, so
    # this stays in sync automatically as that script changes.
    cpu_df = vaccel_epf.apply_seg_remote_override(cpu_df, "cpu")
    gpu_df = vaccel_epf.apply_seg_remote_override(gpu_df, "gpu")

    iso_perf = vaccel_epf.build_perf_map(vaccel_epf.ISO_PERF_FILE)
    e2e_perf = vaccel_epf.build_perf_map(vaccel_epf.E2E_PERF_FILE)

    if vaccel_epf.seg_override_enabled():
        seg_remote_perf = vaccel_epf.build_perf_map(str(vaccel_epf.get_seg_remote_model_summary_path()))
        for key, n in seg_remote_perf.items():
            _, model, backend, _, _ = key
            if vaccel_epf.is_seg_remote(model, backend):
                e2e_perf[key] = n

    if vaccel_epf.WORKLOAD_ONLY:
        idle_power = vaccel_epf.load_idle_power_map(vaccel_epf.IDLE_SUMMARY_FILE)
        robot_idle_w = idle_power.get(vaccel_epf.IDLE_HOST_ROBOT)
        edge_cpu_idle_w = idle_power.get(vaccel_epf.IDLE_HOST_EDGE_CPU)
        edge_gpu_idle_w = idle_power.get(vaccel_epf.IDLE_HOST_EDGE_GPU)
    else:
        robot_idle_w = edge_cpu_idle_w = edge_gpu_idle_w = None

    robot_rows = vaccel_epf.load_robot_cpu_rows(cpu_df, iso_perf, e2e_perf, robot_idle_w)
    edge_cpu_rows = vaccel_epf.load_edge_cpu_remote_rows(cpu_df, e2e_perf, edge_cpu_idle_w)
    edge_gpu_rows = vaccel_epf.load_edge_gpu_remote_rows(gpu_df, e2e_perf, edge_gpu_idle_w)
...
def load_merged_maps_with_debug():
    ...
    robot_debug, edge_cpu_debug, edge_gpu_debug = {}, {}, {}
    ...
    for (model, variant, host, kind), epf in ros2_map.items():
        ...
        if host == "robot" and kind == "cpu":
            ...
            edge_cpu_debug[(model, "ROS2", backend)] = debug_val
        elif host == "edge-asus" and kind == "gpu" and scenario == "Remote GPU":
            edge_gpu_map[(model, "ROS2", backend)] = epf
            edge_gpu_debug[(model, "ROS2", backend)] = debug_val

    return robot_cpu_map, edge_cpu_map, edge_gpu_map, robot_debug, edge_cpu_debug, edge_gpu_debug
...
def load_merged_maps():
    ...
    """
    robot_cpu_map, edge_cpu_map, edge_gpu_map, *_debug = load_merged_maps_with_debug()
...
def print_debug_info(robot_cpu_map, edge_cpu_map, edge_gpu_map):
    """Prints every plotted value (J/frame) to the console, same layout as
    the figure: Robot CPU per scenario, then Edge CPU (Remote CPU only),
    then Edge GPU (Remote GPU only)."""
    present = sorted({m for (m, *_rest) in robot_cpu_map})
    base_models = vaccel_data.ordered_models(present)

    W = 108
    print("\n" + "=" * W)
    print(f"{'DEBUG: PLOTTED ENERGY PER FRAME (J/frame)':^{W}}")
    print("=" * W)
