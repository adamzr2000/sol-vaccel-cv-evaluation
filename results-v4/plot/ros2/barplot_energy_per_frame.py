...
# loaded by explicit path (not a plain import) since comparison/ also has a
# same-named barplot_e2e_latency_and_fps.py -- avoids import collisions.
_e2e_mod = _load_module(Path(__file__).parent / "barplot_e2e_latency_and_fps.py", "_ros2_epf_e2e_source")

# Semantic-segmentation models only: read Remote GPU energy/frame-count
# from an alternate summary file (e.g. a fixed-FPS rerun) instead of the
# default remote-gpu_*.json, applied uniformly to every model in CAT_SEG.
#
# Single source of truth: read from barplot_e2e_latency_and_fps.py -- set
# it there (or via its comparison/ alias) rather than here, so FPS/latency
# and both energy plots stay in sync automatically.
SEG_REMOTE_GPU_TAG_OVERRIDE: str | None = _e2e_mod.SEG_REMOTE_GPU_TAG_OVERRIDE
...
def load_frame_counts() -> dict[tuple, int]:
    ...
    for tag in ("local-cpu", "remote-cpu", "remote-gpu"):
        ...
        for run in json.loads(path.read_text())["runs"]:
            ...
            # Segmentation models read their Remote GPU frame count from
    ...
    # canonical "remote-gpu" tag (not that file's own run_tag) so
    # load_epf_map()'s lookup -- which also forces "remote-gpu" for these
    # models -- finds it.
    if SEG_REMOTE_GPU_TAG_OVERRIDE:
        ov_path = MODEL_DIR / f"{SEG_REMOTE_GPU_TAG_OVERRIDE}_model_stats.json"
        if not ov_path.exists():
            print(f"[WARN] Missing override model-stats: {ov_path}")
        else:
            for run in json.loads(ov_path.read_text())["runs"]:
                model = run.get("model", "")
                if model not in CAT_SEG:
                    continue
                backend = run.get("backend", "")
                n = run.get("n_frames_monitor")
                if n:
                    counts[(model, backend, "remote-gpu")] = int(n)
...
def load_epf_map(frame_counts: dict[tuple, int]) -> dict[tuple, float]:
    ...
    EPF = (mean_power_W × duration_sec) / n_frames_monitor
        = total_energy_J_over_experiment / frames_processed_on_the_robot
    """
    ...
    # matched against the override frame count stored under the
    # "remote-gpu" tag above.
    if SEG_REMOTE_GPU_TAG_OVERRIDE:
        ov_path = STATS_DIR / f"{SEG_REMOTE_GPU_TAG_OVERRIDE}_system_stats.json"
        if not ov_path.exists():
            ...
            for run in json.loads(ov_path.read_text())["runs"]:
                ...
                if model not in CAT_SEG:
                    continue
                backend = run["backend"]
                label = _classify(backend, "remote-gpu")
                if not label:
                    continue
                n = frame_counts.get((model, backend, "remote-gpu"))
                if not n:
                    print(f"[WARN] No override frame count for ({model}, {backend}, remote-gpu) — skipped")
                    continue
                for host, host_data in run["hosts"].items():
                    for kind, e_key in [("cpu", "cpu_energy_j"), ("gpu", "gpu_energy_j")]:
                        if kind not in host_data:
