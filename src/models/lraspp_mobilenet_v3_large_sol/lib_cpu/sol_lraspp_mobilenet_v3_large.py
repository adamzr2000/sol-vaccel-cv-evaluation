...
class sol_lraspp_mobilenet_v3_large:
	def open(self):
		self.lib = ctypes.CDLL(self.path + "/" + "libsol_lraspp_mobilenet_v3_large.so")

		self.call = self.lib.__getattr__("sol_predict") 
		self.call.restype = None

		self.init = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_init")
		self.init.restype = None
		self.init.argtypes = None

		self.free = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_free")
		self.free.restype = None
		self.free.argtypes = None

		self.seed_ = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_set_seed")
		self.seed_.argtypes = [ctypes.c_uint64]
		self.seed_.restype = None

		self.set_IO_ = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_set_IO")
		self.set_IO_.restype = None

		self.call_no_args = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_run")
		self.call_no_args.argtypes = None
		self.call_no_args.restype = None

		self.get_output = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_get_output")
		self.get_output.argtypes = None
		self.get_output.restype = None

		self.sync = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_sync")
		self.sync.argtypes = None
		self.sync.restype = None

		self.opt_ = self.lib.__getattr__("sol_lraspp_mobilenet_v3_large_optimize")
	...
	def close(self):
		dlclose_func = ctypes.CDLL(self.path + "/" + "libsol_lraspp_mobilenet_v3_large.so").dlclose
...
def run_example():
	...
	out__0_out = np.zeros((vdims_0, 21, 224, 224), dtype=np.float32)
	dp_args = [in__input, out__0_out, vdims] # Inputs, Outputs, VDims must be in this exact order!
