#!/usr/bin/env python3
"""
barplot_system_stats_cpu.py

Barplots from system-stats CPU summary:
  - Edge CPU utilization (%)
  - Edge CPU power (W)
  - Robot CPU utilization (%)
  - Robot CPU power (W)

Input:
  ../experiments/system-stats/_summary/run1_overall_cpu_stats.csv
...
Variant split:
  - model.endswith("_sol") -> SOL
  - else -> PyTorch

Output control:
  - PLOT_MODE = "combined": 4 stacked panels into OUTPUT_COMBINED
  - PLOT_MODE = "separate": one PDF per plot (see PLOTS)
"""
...
OUTPUT_COMBINED = "system_stats_cpu.pdf"  # used when PLOT_MODE == "combined"
FIG_SIZE_COMBINED = (10.5, 14.5)  # 4 stacked panels
...
PLOTS = [
    dict(
        out="system_stats_cpu_edge_utilization.pdf",
        host="edge",
    ...
    dict(
        out="system_stats_cpu_edge_power.pdf",
        host="edge",
        y="cpu_watts_mean",
        yerr="cpu_watts_std",
        ylabel="Power (W)",
    ...
    dict(
        out="system_stats_cpu_robot_utilization.pdf",
        host="robot",
        y="cpu_util_percent_mean",
        yerr="cpu_util_percent_std",
        ylabel="CPU (%)",
    ...
    dict(
        out="system_stats_cpu_robot_power.pdf",
        host="robot",
        y="cpu_watts_mean",
        yerr="cpu_watts_std",
        ylabel="Power (W)",
...
def main():
    ...
    # Keep only stock CPU rows (both edge + robot)
    df = df[(df["backend"] == "stock") & (df["device"] == "cpu")].copy()
    if df.empty:
        raise SystemExit("No rows after filtering backend='stock', device='cpu'.")
    ...
    if PLOT_MODE == "separate":
        for cfg in PLOTS:
            ...
            if sub.empty:
                ...
                continue
            plot_metric(sub, cfg["y"], cfg["yerr"], cfg["ylabel"], cfg["title"], ax=None, out_file=cfg["out"])
        return

    # combined: 4 stacked panels, independent y-scales (plot_metric computes its own)
    ...
    for ax, cfg in zip(axes, PLOTS):
        ...
        if sub.empty:
            ax.axis("off")
            ax.text(0.5, 0.5, f"No data for host={cfg['host']} (stock cpu)",
                    ha="center", va="center", transform=ax.transAxes)
            continue
        plot_metric(sub, cfg["y"], cfg["yerr"], cfg["ylabel"], cfg["title"], ax=ax, out_file=None)
