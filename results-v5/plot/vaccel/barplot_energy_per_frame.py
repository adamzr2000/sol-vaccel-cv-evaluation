...
Energy per frame = cpu_energy_j_total / gpu_energy_j_total (exact hardware-
counter totals from summarize_system_stats.py, see barplot_energy_consumption.py)
minus the corresponding idle baseline (idle/summary.csv's exact counter
power, scaled to this run's own duration_sec), divided by num_processed_frames
(from the benchmark JSON). No per-run std is available for this figure -
it's a single exact total, not a distribution.
...
from plot_config import (
    get_path, load_config, get_model_type_order, get_model_display_name,
    apply_seg_remote_override, seg_override_enabled, is_seg_remote,
    get_seg_remote_model_summary_path,
)
...
def build_perf_map(json_path: str) -> dict:
    ...
    for r in runs:
        ...
        # r.get("remote_host", "") only falls back to "" if the key is absent -
        # local (non-remote) runs have the key present with an explicit JSON
        # null, which .get() returns as None, so str(None) -> "none" and
        # every local-run lookup (which expects "") would silently miss.
        remote_host = str(r.get("remote_host") or "").lower().strip()
...
def load_idle_power_map(csv_path: str) -> dict[str, float]:
    """
    ...
    exact cpu/gpu_energy_j_total values being corrected here.
    """
    path = Path(csv_path)
    if not path.exists():
...
def _subtract_idle(j: float, duration_sec: float, idle_power_w: float | None) -> float:
    ...
    if idle_power_w is None or not np.isfinite(idle_power_w) or not np.isfinite(duration_sec):
        return j
...
def load_robot_cpu_rows(cpu_df: pd.DataFrame,
                        iso_perf: dict, e2e_perf: dict,
                        idle_power_w: float | None) -> list:
...
def load_edge_cpu_remote_rows(cpu_df: pd.DataFrame, e2e_perf: dict,
                              idle_power_w: float | None) -> list:
    rows = []
...
def load_edge_gpu_remote_rows(gpu_df: pd.DataFrame, e2e_perf: dict,
                              idle_power_w: float | None) -> list:
    rows = []
...
def print_debug_table(panels, base_models):
    ...
    for panel_key, ylabel, rows_data, vars_present in panels:
        print(f"\n--- {panel_key.upper()} ---")
        print(f"{'Model':<22} | {'Variant':<28} | {'J/frame':>10} | {'Std':>10} | {'Energy(J)':>10} | {'Frames':>7}")
        print("-" * 100)
        ...
        energy_map  = {(m, v): np.nan for m in base_models for v in vars_present}
        frames_map  = {(m, v): None   for m in base_models for v in vars_present}
        for r in rows_data:
            mean_map  [(r["base_model"], r["variant"])] = r["mean"]
            std_map   [(r["base_model"], r["variant"])] = r["std"]
            energy_map[(r["base_model"], r["variant"])] = r.get("energy_j", np.nan)
            frames_map[(r["base_model"], r["variant"])] = r.get("n_frames")
        for m in base_models:
            for v in vars_present:
                ...
                if np.isfinite(val):
                    ...
                    e = energy_map[(m, v)]
                    e_str = f"{e:10.2f}" if np.isfinite(e) else f"{'N/A':>10}"
                    n = frames_map[(m, v)]
                    n_str = f"{n:7d}" if n else f"{'N/A':>7}"
                    print(f"{m:<22} | {v:<28} | {val:10.4f} | {std_str} | {e_str} | {n_str}")
    print("\n" + "=" * 95 + "\n")
...
def main():
    ...
    cpu_df = apply_seg_remote_override(cpu_df, "cpu")
    gpu_df = apply_seg_remote_override(gpu_df, "gpu")
    ...
    # num_processed_frames for segmentation-model remote rows must come from
    # the same overridden run as the energy data above, not the base e2e run.
    if seg_override_enabled():
        seg_remote_perf = build_perf_map(str(get_seg_remote_model_summary_path()))
        for key, n in seg_remote_perf.items():
            _, model, backend, _, _ = key
            if is_seg_remote(model, backend):
                e2e_perf[key] = n

    if WORKLOAD_ONLY:
        idle_power = load_idle_power_map(IDLE_SUMMARY_FILE)
        robot_idle_w    = idle_power.get(IDLE_HOST_ROBOT)
        edge_cpu_idle_w = idle_power.get(IDLE_HOST_EDGE_CPU)
        edge_gpu_idle_w = idle_power.get(IDLE_HOST_EDGE_GPU)
        print(f"Idle baseline (W): robot={robot_idle_w}, edge_cpu={edge_cpu_idle_w}, edge_gpu={edge_gpu_idle_w}")
    else:
        robot_idle_w = edge_cpu_idle_w = edge_gpu_idle_w = None
        print("WORKLOAD_ONLY=False - plotting raw energy totals (no idle correction).")

    robot_rows    = load_robot_cpu_rows(cpu_df, iso_perf, e2e_perf, robot_idle_w)
    edge_cpu_rows = load_edge_cpu_remote_rows(cpu_df, e2e_perf, edge_cpu_idle_w)
    edge_gpu_rows = load_edge_gpu_remote_rows(gpu_df, e2e_perf, edge_gpu_idle_w)
