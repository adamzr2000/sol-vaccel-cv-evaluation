#!/usr/bin/env python3
"""
barplot_local_latency_isolated_simplified.py

Local inference latency (mean ± std dev) from iso runs.
3 rows (deployment targets) × 3 columns (model categories).

Simplified from barplot_local_latency_isolated.py: drops all vAccel RPC (*)
backends and the vAccel Local vs. RPC connector trend lines (blue/green) -
only native vs. vAccel-Local comparisons remain.

5 backends — ordering: PTC first (JIT baseline), then AOTI family, then SOL family:
  PTC  |  AOTI · vAccel Local+PTC  |  SOL · vAccel Local+SOL
...
OUTPUT_FILE = _HERE / "iso_local_latency_simplified.pdf"
...
# AOTI family: dark blue (AOT-Inductor) / medium blue (vAccel Local)
# SOL  family: dark green (SOL) / medium green (vAccel Local)
COLOR_MAP = {
...
def main():
    ...
    for row_idx, (host, device, row_label) in enumerate(ROWS):
        for col_idx, (cat_title, models) in enumerate(CATEGORIES):
            ...
            # bar_pos[(backend, model_idx)] = (x_center, y_val) — used for family connectors
            bar_pos: dict = {}
    ...
    # bp[0]=JIT  bp[1]=AOT  bp[2]=vAccelLocalAOT  bp[3]=SOL  bp[4]=vAccelLocalSOL
