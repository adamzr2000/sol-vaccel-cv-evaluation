...
energy_kJ = cpu_energy_j / gpu_energy_j (from system-stats _summary JSONs) / 1000
...
_e2e_mod = _load_module(Path(__file__).parent / "barplot_e2e_latency_and_fps.py", "_ros2_energy_e2e_source")
...
# Semantic-segmentation models only: read Remote GPU energy from an
# alternate summary file (e.g. a fixed-FPS rerun, see
# ../../experiments/system-stats/ros2/_summary/) instead of the default
# remote-gpu_system_stats.json, applied uniformly to every model in
# CAT_SEG.
#
...
def load_energy_map() -> dict[tuple, float]:
    """
    Returns {(model, variant_label, host, kind): energy_kJ}.
    """
        ...
        for run in json.loads(path.read_text())["runs"]:
            ...
            for host, host_data in run["hosts"].items():
                for kind, e_key in [("cpu", "cpu_energy_j"), ("gpu", "gpu_energy_j")]:
                    ...
                    if ej is not None:
                        energy_map[(model, label, host, kind)] = float(ej) / 1000.0
    ...
    if SEG_REMOTE_GPU_TAG_OVERRIDE:
        ...
        if not ov_path.exists():
            ...
            for run in json.loads(ov_path.read_text())["runs"]:
                if run["model"] not in CAT_SEG:
                    continue
                label = _classify(run["backend"], "remote-gpu")
                if not label:
                    continue
                for host, host_data in run["hosts"].items():
                    for kind, e_key in [("cpu", "cpu_energy_j"), ("gpu", "gpu_energy_j")]:
                        ...
                        ej = host_data[kind].get(e_key)
                        if ej is not None:
                            energy_map[(run["model"], label, host, kind)] = float(ej) / 1000.0
